...
import { Database, FileUp, Loader2, Plus, Edit2, Trash2, Search, Tag, X, Save, List, Eye, AlertCircle, FileDown } from 'lucide-react';
import * as XLSX from 'xlsx';
const CatalogoInstrumentos = ({ darkMode }) => {
  ...
  const handleGuardarManual = async (e) => {
    ...
    try {
      ...
      alert(`Error al guardar instrumento: ${serverMsg}`); 
    }
  };
  ...
  const handleExportarExcel = () => {
    if (instrumentos.length === 0) return alert("No hay datos para exportar");
    const ws = XLSX.utils.json_to_sheet(instrumentos.map(item => ({
      'ID': item.id,
      'Nombre de Instrumento': item.nombre
    })));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Instrumentos");
    XLSX.writeFile(wb, "Instrumentos_Export.xlsx");
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6 border-opacity-20 border-[#C9EA63]">
        ...
        <div>
          <h2 className={`text-3xl font-bold flex items-center gap-3 ${textTitle}`}>
            <List className={darkMode ? 'text-[#C9EA63]' : 'text-emerald-600'} size={32} /> 
            Catálogo de Instrumentos
          </h2>
          <p className={`mt-2 text-sm ${darkMode ? 'text-[#F2F6F0]/70' : 'text-gray-500'}`}>
            Tipos de instrumentos que el laboratorio tiene acreditados para calibrar.
          </p>
        </div>
        ...
        <div className="flex items-start gap-3 flex-wrap md:flex-nowrap justify-end">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              <p className={`text-[10px] text-right mt-1 max-w-[200px] leading-tight ${darkMode ? 'text-[#F2F6F0]/60' : 'text-slate-500'}`}>
                El Excel debe tener una columna obligatoria <b>nombre</b>. 
              </p>
            </div>
        </div>
      </div>
      ...
      <div className={`rounded-2xl border p-6 transition-colors duration-300 ${boxBg}`}>
        <div className="mb-6 flex gap-4">
            <div className={`flex items-center gap-2 w-full max-w-md px-4 py-2 border rounded-xl ${inputBg}`}>
                ...
                <input 
                    type="text" 
                    placeholder="Buscar tipo de instrumento..." 
            ...
            <table className="w-full text-sm text-left">
                <thead className={`text-xs uppercase border-b ${darkMode ? 'bg-[#141f0b] text-[#C9EA63]' : 'bg-slate-100 text-slate-600'}`}>
                    <tr>
                        ...
                        <th className="px-4 py-4">Nombre del Instrumento</th>
                        ...
                        <tr key={item.id} className={`transition-colors ${darkMode ? 'bg-[#253916] hover:bg-[#314a1c]' : 'bg-white hover:bg-emerald-50/50'}`}>
                            ...
                            <td className="px-4 py-3 text-center">
                                <div className="flex justify-center gap-2">
                                    ...
                                    <button onClick={() => abrirModalEditar(item)} className={`p-2 rounded-lg transition-colors ${darkMode ? 'hover:bg-[#141f0b] text-[#C9EA63]' : 'hover:bg-emerald-100 text-emerald-600'}`}><Edit2 size={16}/></button>
                    ...
                    {instrumentosFiltrados.length === 0 && (
                        <tr>
                            <td colSpan="3" className="px-4 py-8 text-center opacity-60">
                                {instrumentos.length === 0 ? 'No hay instrumentos registrados.' : 'No hay resultados para la búsqueda.'}
                            </td>
                        </tr>
                    )}
                </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[100]">
          <div className={`p-8 rounded-2xl shadow-2xl w-full max-w-md relative border-t-4 ${darkMode ? 'bg-[#141f0b] border-[#C9EA63]' : 'bg-white border-emerald-600'}`}>
            ...
            <form onSubmit={handleGuardarManual} className="space-y-4">
              <div>
                <label className={`block text-sm font-semibold mb-1 ${darkMode ? 'text-[#F2F6F0]/80' : 'text-gray-600'}`}>Nombre / Tipo de Instrumento *</label>
      ...
      {editandoItem && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[100]">
          <div className={`p-8 rounded-2xl shadow-2xl w-full max-w-md relative border-t-4 ${darkMode ? 'bg-[#141f0b] border-yellow-400' : 'bg-white border-yellow-500'}`}>
            ...
            <h2 className={`text-2xl font-bold mb-6 flex items-center gap-2 ${darkMode ? 'text-[#F2F6F0]' : 'text-slate-800'}`}>
              <Edit2 className={darkMode ? 'text-yellow-400' : 'text-yellow-500'} /> Editar Instrumento
            </h2>
      ...
      {viendoItem && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[100]">
          <div className={`p-8 rounded-2xl shadow-2xl w-full max-w-md relative border-t-4 ${darkMode ? 'bg-[#141f0b] border-blue-400' : 'bg-white border-blue-500'}`}>
            ...
            <h2 className={`text-2xl font-bold mb-6 flex items-center gap-2 ${darkMode ? 'text-[#F2F6F0]' : 'text-slate-800'}`}>
              <Eye className={darkMode ? 'text-blue-400' : 'text-blue-500'} /> Detalle de Instrumento
            </h2>
            <div className="space-y-4">
              <div>
                <label className={`block text-xs font-semibold mb-1 uppercase ${darkMode ? 'text-[#F2F6F0]/50' : 'text-gray-400'}`}>Nombre / Tipo</label>
      ...
      {modalConfirmarVaciar && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[100]">
          <div className={`p-8 rounded-2xl shadow-2xl w-full max-w-sm relative ${darkMode ? 'bg-[#1b2b10] border border-rose-900/50' : 'bg-white border border-rose-100'}`}>
            ...
            <p className={`text-center text-sm mb-6 ${darkMode ? 'text-[#F2F6F0]/70' : 'text-slate-500'}`}>Estás a punto de eliminar <b>TODOS</b> los registros de instrumentos. Esta acción es irreversible.</p>
      ...
      {modalConfirmarEliminar && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[100]">
          <div className={`p-8 rounded-2xl shadow-2xl w-full max-w-sm relative ${darkMode ? 'bg-[#1b2b10] border border-rose-900/50' : 'bg-white border border-rose-100'}`}>
            ...
            <h2 className={`text-2xl font-black text-center mb-2 ${darkMode ? 'text-[#F2F6F0]' : 'text-slate-800'}`}>¿Eliminar Instrumento?</h2>
...
export default CatalogoInstrumentos;
